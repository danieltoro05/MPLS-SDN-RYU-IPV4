...
class RestRouterAPI(app_manager.RyuApp):
    ...
    def _stats_reply_handler(self, ev):
        ...
        if (dp.id not in self.waiters or
           msg.xid not in self.waiters[dp.id]):
            return
...
class Router(dict):
    ...
    def _del_vlan_router(self, vlan_id, waiters):
        ...
        if (len(vlan_router.address_data) == 0 and
                len(vlan_router.routing_tbl) == 0):
    ...
    def _chk_addr_relation_route(self, address_id):
        ...
        for gateway in gateways:
            ...
            if address is not None:
                if (address_id == REST_ALL and
                   address.address_id not in relate_list):
